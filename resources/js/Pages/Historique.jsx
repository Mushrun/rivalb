...
function AvisModal({ name, onClose }) {
    ...
    if (sent) return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4"
            ...
            <div className="w-full max-w-[390px] rounded-2xl p-6 flex flex-col items-center gap-3"
                ...
                <p className="text-white font-bold">{t('historique.review_sent')}</p>
                <p className="text-[#888] text-xs text-center">
                    {t('historique.review_sent_sub', { name })}
                </p>
                <button onClick={onClose} className="w-full rounded-xl py-3 font-bold text-sm mt-1"
                    style={{ background: '#FF3B30', color: '#FFF' }}>{t('historique.close')}</button>
            </div>
        </div>
    );
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4"
            ...
            <div className="w-full max-w-[390px] rounded-2xl p-5"
                ...
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-white font-bold text-base">{t('historique.review_title')} {name}</h3>
                ...
                <div className="flex gap-3 mb-4">
                    {[
                        ...
                        <button key={item.key} onClick={() => setSentiment(item.key)}
                            ...
                            style={{
                                background: sentiment === item.key ? item.bg : '#0D0D0D',
                                border: sentiment === item.key ? `2px solid ${item.active}` : '2px solid #2A2A2A',
                                color: sentiment === item.key ? item.active : '#666',
                            }}>
                            {item.label}
                        </button>
                    ))}
                ...
                <textarea value={comment} onChange={e => setComment(e.target.value)}
                    placeholder={t('historique.review_comment')} rows={3}
...
function CombatsTab({ combats, stats }) {
    ...
    const filters = [
        { key: 'ALL',  label: t('historique.filter_all') },
        { key: 'WON',  label: t('historique.filter_won') },
        { key: 'LOST', label: t('historique.filter_lost') },
    ];

    const filtered = filter === 'ALL'  ? combats
        : filter === 'WON'             ? combats.filter(c => c.won)
    ...
    return (
        <div>
            <div className="flex gap-2 px-4 mb-4">
                {[
                    { label: t('historique.wins'),   value: stats.wins,   color: '#4CD964' },
                    { label: t('historique.losses'),  value: stats.losses, color: '#FF3B30' },
                    { label: t('historique.balance_rb'), value: bilanLabel, color: bilanColor },
                ].map(s => (
                    ...
                    <div key={item.id} className="flex items-center gap-3 rounded-2xl p-3.5"
                        ...
                        <div className="flex flex-col items-end gap-1">
                            ...
                            {avisGiven.has(item.id) ? (
                                <span className="text-[#4CD964] text-[10px] font-semibold">{t('historique.review_given')}</span>
                            ) : (
                                <button onClick={() => setAvisModal(item)}
                                    className="text-[10px] font-bold px-2 py-0.5 rounded-md"
                                    style={{ background: 'rgba(255,170,136,0.12)', color: '#FFAA88' }}>
                                    {t('historique.leave_review')}
                                </button>
                            )}
                        </div>
                    </div>
...
function TransactionsTab({ transactions, balanceRb }) {
    ...
    const filters = [
        { key: 'ALL',          label: t('historique.filter_all') },
        { key: 'DEPOSITS',     label: t('historique.filter_deposits') },
        { key: 'WITHDRAWALS',  label: t('historique.filter_withdrawals') },
    ];

    const filtered = filter === 'ALL'         ? transactions
        : filter === 'DEPOSITS'               ? transactions.filter(t => t.type === 'recharge')
    ...
    return (
        <div>
            <div className="mx-4 mb-4 rounded-2xl p-4 flex items-center justify-between"
                style={{ background: '#1A1A1A' }}>
                <div>
                    <p className="text-[#888] text-xs mb-1">{t('historique.current_balance')}</p>
...
export default function Historique({ combats = [], transactions = [], stats = { wins: 0, losses: 0, total_rb: 0 }, balance_rb = 0 }) {
    ...
    const tabs = [
        { key: 'COMBATS',      label: t('historique.tab_combats') },
        { key: 'TRANSACTIONS', label: t('historique.tab_transactions') },
    ];
            ...
            <div className="flex px-4 mt-3 mb-4 border-b" style={{ borderColor: '#1E1E1E' }}>
                {tabs.map(item => (
                    <button key={item.key} onClick={() => setTab(item.key)}
                        ...
                        style={{
                            color: tab === item.key ? '#FF3B30' : '#555',
                            borderBottom: tab === item.key ? '2px solid #FF3B30' : '2px solid transparent',
                        }}>
                        {item.label}
                    </button>
                ))}
