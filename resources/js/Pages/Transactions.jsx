...
export default function Transactions() {
    ...
    const filtered = filter === 'ALL'      ? transactions
        : filter === 'DEPOSITS'            ? transactions.filter(tx => tx.type === 'recharge')
        : transactions.filter(tx => tx.type === 'retrait');

    const totalRB = transactions.reduce((sum, tx) => {
        const val = parseInt(tx.amount.replace(/[^0-9-]/g, ''));
        return sum + val;
    }, 0);
    ...
    return (
        <AppLayout>
            ...
            <div className="flex items-center gap-3 px-4 pt-4 pb-2">
                ...
                <h1 className="text-white font-bold text-lg">{t('historique.transactions_title')}</h1>
            </div>
            ...
            <div className="mx-4 mt-3 rounded-2xl p-4 flex items-center justify-between"
                ...
                <div>
                    <p className="text-[#888888] text-xs mb-1">{t('historique.current_balance')}</p>
                    ...
                    <div key={tx.id}
                        ...
                        <div className="flex-1">
                            <p className="text-white font-semibold text-sm">
                                {tx.type === 'recharge' ? t('historique.deposit') : t('historique.withdrawal')}
                            </p>
